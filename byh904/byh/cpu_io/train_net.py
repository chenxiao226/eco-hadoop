...
from build_cpuio_dataset_from_generated import CPUIODataset
...
class Trainer:
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, args.NUM_EPOCHS_1 + 1):
            ...
            for batched_features, batched_targets1, batched_targets2 in self.data_loader:
                ...
                #print("batched_outputs1:",batched_outputs1)
                #print("batched_outputs2:",batched_outputs2)
                #print("loss1:",loss1)
                #print("loss2:",loss2)
    ...
    def test(
        ...
        random_choice = np.random.choice(range(len(all_inverse_outputs1)), size=min(100, len(all_inverse_outputs1)), replace=False)
        ...
        # all_outputs2 = all_outputs2.cpu().numpy() * 100
        # all_targets2 = all_targets2.cpu().numpy() * 100
        # for i in range(all_outputs2.shape[0]):
        #     fig = plt.figure(figsize=(20, 20))
        #     ax = fig.add_subplot(111, projection='3d')

        #     xpos, ypos = np.meshgrid(range(len(cfg.CPU_SLICES)), range(len(cfg.SDA_SLICES)), indexing="ij")
        #     xpos = xpos.ravel()
        #     ypos = ypos.ravel()

        #     ax.bar3d(xpos, ypos, 0, 0.5, 1, all_targets2[i].ravel(), alpha=0.8, color='g', label="Actual Time Ratio")
        #     ax.bar3d(xpos + 0.5, ypos, 0, 0.5, 1, all_outputs2[i].ravel(), alpha=0.8, color='r', label="Predict Time Ratio")

        #     ax.set_xticks(range(len(cfg.CPU_SLICES) + 1), np.array(cfg.CPU_SLICES + [cfg.CPU_UPPER_BOUND - 1], dtype=int), fontsize=15)
        #     ax.set_yticks(range(len(cfg.SDA_SLICES) + 1), np.array(cfg.SDA_SLICES + [cfg.SDA_UPPER_BOUND], dtype=int), fontsize=15)
        #     ax.set_xlabel('CPU usage (%)', fontsize=15)
        #     ax.set_ylabel('SDA usage (%)', fontsize=15)
        #     ax.set_zlabel('Time Ratio (%)', fontsize=15)
        #     ax.set_title("Predict and Actual Time Ratio", fontsize=20)
        #     ax.legend(fontsize=15)
        #     plt.savefig(args.output_dir + "/vis_model_final2_{}.jpg".format(i))
        #     plt.close()
