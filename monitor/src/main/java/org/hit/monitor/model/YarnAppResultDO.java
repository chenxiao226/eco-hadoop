...
/**
 * @ClassName: YarnAppResult
 * 
 */
public class YarnAppResultDO {

	private String id;// The application id, e.g.,
						// application_1236543456321_1234567 ( 主键 )
	...
	private String trackingUrl;// The web URL that can be used to track the
								// application
	private String jobType;// The Job Type e.g, Pig, Hive, Spark, HadoopJava
	private Integer severity;// Aggregate severity of all the heuristics. Ranges
								// from 0(LOW) to 4(CRITICAL)
	private Integer score;// The application score which is the sum of heuristic
							// scores
	private Integer workflowDepth;// The application depth in the scheduled
									// flow. Depth starts from 0
	private String scheduler;// The scheduler which triggered the application
	private String jobName;// The name of the job in the flow to which this app
							// belongs
	private String jobExecId;// A unique reference to a specific execution of
								// the job/action(job in the workflow). This
								// should filter all applications
								// (mapreduce/spark) triggered by the job for a
								// particular execution.
	private String flowExecId;// A unique reference to a specific flow
								// execution. This should filter all
								// applications fired by a particular flow
								// execution. Note that if the scheduler
								// supports sub-workflows, then this ID should
								// be the super parent flow execution id that
								// triggered the the applications and
								// sub-workflows.
	private String jobDefId;// A unique reference to the job in the entire flow
							// independent of the execution. This should filter
							// all the applications(mapreduce/spark) triggered
							// by the job for all the historic executions of
							// that job.
	private String flowDefId;// A unique reference to the entire flow
								// independent of any execution. This should
								// filter all the historic mr jobs belonging to
								// the flow. Note that if your scheduler
								// supports sub-workflows, then this ID should
								// reference the super parent flow that
								// triggered the all the jobs and sub-workflows.
	...
	private Long totalDelay;// The total delay in starting of mappers and
							// reducers

	/**
	 * 以下属性是业务逻辑属性
	 */
	private List<YarnAppHeuristicResultDO> yarnAppHeuristicResultDOList;

	public List<YarnAppHeuristicResultDO> getYarnAppHeuristicResultDOList() {
		return yarnAppHeuristicResultDOList;
	}

	/**
	 * 
	 * @param yarnAppHeuristicResultDOList
	 */
	...
	public void setYarnAppHeuristicResultDOList(List<YarnAppHeuristicResultDO> yarnAppHeuristicResultDOList) {
		this.yarnAppHeuristicResultDOList = yarnAppHeuristicResultDOList;
	}
	...
	/**
	 * 获取 Aggregate severity of all the heuristics. Ranges from 0(LOW) to
	 * 4(CRITICAL)  
	 */
	...
	/**
	 * 设定 Aggregate severity of all the heuristics. Ranges from 0(LOW) to
	 * 4(CRITICAL)  
	 */
	...
	/**
	 * 获取 A unique reference to a specific execution of the job/action(job in
	 * the workflow). This should filter all applications (mapreduce/spark)
	 * triggered by the job for a particular execution.  
	 */
	...
	/**
	 * 设定 A unique reference to a specific execution of the job/action(job in
	 * the workflow). This should filter all applications (mapreduce/spark)
	 * triggered by the job for a particular execution.  
	 */
	...
	/**
	 * 获取 A unique reference to a specific flow execution. This should filter
	 * all applications fired by a particular flow execution. Note that if the
	 * scheduler supports sub-workflows, then this ID should be the super parent
	 * flow execution id that triggered the the applications and sub-workflows.
	 *  
	 */
	...
	/**
	 * 设定 A unique reference to a specific flow execution. This should filter
	 * all applications fired by a particular flow execution. Note that if the
	 * scheduler supports sub-workflows, then this ID should be the super parent
	 * flow execution id that triggered the the applications and sub-workflows.
	 *  
	...
	/**
	 * 获取 A unique reference to the job in the entire flow independent of the
	 * execution. This should filter all the applications(mapreduce/spark)
	 * triggered by the job for all the historic executions of that job.  
	 */
	...
	/**
	 * 设定 A unique reference to the job in the entire flow independent of the
	 * execution. This should filter all the applications(mapreduce/spark)
	 * triggered by the job for all the historic executions of that job.  
	 */
	...
	/**
	 * 获取 A unique reference to the entire flow independent of any execution.
	 * This should filter all the historic mr jobs belonging to the flow. Note
	 * that if your scheduler supports sub-workflows, then this ID should
	 * reference the super parent flow that triggered the all the jobs and
	 * sub-workflows.  
	 */
	...
	/**
	 * 设定 A unique reference to the entire flow independent of any execution.
	 * This should filter all the historic mr jobs belonging to the flow. Note
	 * that if your scheduler supports sub-workflows, then this ID should
	 * reference the super parent flow that triggered the all the jobs and
	 * sub-workflows.  
