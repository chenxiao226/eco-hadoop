...
public abstract class GenericMemoryHeuristic implements Heuristic<MapReduceApplicationData> {
  ...
  private double[] memoryLimits = {1.1d, 1.5d, 2.0d, 2.5d};   //Container Memory Seve rity Limits
...
//        _heuristicConfData.getHeuristicName(), severity, Utils.getHeuristicScore(severity, tasks.length));
    double ratio = ((double)taskPMemAvg) / containerMem;
    
    HeuristicResult result = new HeuristicResult(_heuristicConfData.getClassName(),
    		_heuristicConfData.getHeuristicName(), severity, getScore(ratio));
  ...
  /** 具体得分 **/
	private int getScore(double ratio) {
		return  (int) (((double)ratio/0.2))*(-5)+100;
	}
}
