...
import com.linkedin.drelephant.StartHistoryAnalyzeTime;
...
/**
 * This class provides a list of analysis promises to be generated under Hadoop
 * YARN environment
 */
public class AnalyticJobGeneratorHadoop2 implements AnalyticJobGenerator {
	...
	// Generate a token update interval with a random deviation so that it does
	// not update the token exactly at the same
	// time with other token updaters (e.g. ElephantFetchers).
	// 生成一个令牌更新间隔的随机偏差,以便它不更新完全令牌同时与其他令牌更新(例如ElephantFetchers)。
	private static final long TOKEN_UPDATE_INTERVAL = Statistics.MINUTE_IN_MS * 30
			+ new Random().nextLong() % (3 * Statistics.MINUTE_IN_MS);

	private String _resourceManagerAddress;
	private long _lastTime =StartHistoryAnalyzeTime.MR_Last_currentTime;
	...
	/**
	 * Fetch all the succeeded and failed applications/analytic jobs from the
	 * resource manager.
	 *
	 * @return
	 * @throws IOException
	...
	public List<AnalyticJob> fetchAnalyticJobs() throws IOException, AuthenticationException {
		...
		// There is a lag of job data from AM/NM to JobHistoryServer HDFS, we
		// shouldn't use the current time, since there
		// might be new jobs arriving after we fetch jobs. We provide one minute
		// delay to address this lag.
		// 有滞后的工作数据从AM / NM JobHistoryServer
		// HDFS,我们不应该用当前时间,因为可能会有新的工作岗位后到达我们获取工作。我们提供一分钟推迟解决这个滞后。

		_currentTime = System.currentTimeMillis() - FETCH_DELAY;

		updateAuthToken(); // 好像是一个简单的认证

		logger.info("Fetching recent finished application runs between last time: " + (_lastTime + 1)
				+ ", and current time: " + _currentTime);

		//_resourceManagerAddress = "192.168.130.18:8088";
		_resourceManagerAddress =  CONFIG.YARN_RESOURCE_MANAGER.substring(7, 26);
		// CONFIG.YARN_RESOURCE_MANAGER.substring(7, 26);
		//_resourceManagerAddress = CONFIG.RESOURCE_MANAGER.substring(7, 27);
		
		// Fetch all succeeded apps
		URL succeededAppsURL = new URL(new URL("http://" + _resourceManagerAddress),
				String.format("/ws/v1/cluster/apps?finalStatus=SUCCEEDED&finishedTimeBegin=%s&finishedTimeEnd=%s",
		...
		// finalStatus: Status of the Application as reported by the Application
		// Master
		URL failedAppsURL = new URL(new URL("http://" + _resourceManagerAddress),
				String.format(
	 ...
	 * @throws AuthenticationException
	 *             Authencation problem
	 */
	private JsonNode readJsonNode(URL url) throws IOException, AuthenticationException {
	 ...
	 * @throws IOException
	 * @throws AuthenticationException
	 *             Problem authenticating to resource manager
	 */
	private List<AnalyticJob> readApps(URL url) throws IOException, AuthenticationException {
		...
		for (JsonNode app : apps) {
			...
			// When called first time after launch, hit the DB and avoid
			// duplicated analytic jobs that have been analyzed
			// before.
			// AppResult.find.byId(appId) 应该是读取数据库为null的意思
			// AppResult.find.byId(appId)
			if (_lastTime > 0 || (_lastTime == 0)) {
				...
				ApplicationType type = ElephantContext.instance()
						.getApplicationTypeForName(app.get("applicationType").getValueAsText());
