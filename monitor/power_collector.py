"""
功率数据采集脚本
功能：读取功率计导出的 CSV 文件，将有功功率数据写入 MySQL bd_monitor 数据库
...
部署位置二选一（决定 DB_CONFIG.host 和 CSV_FILES 路径）：
  A. 脚本跑在甲方 131 上（推荐，和 cluster_collector.py 同机）
     - host 用 'localhost'
     - CSV 用 Linux 路径，如 /root/power/master.csv（需把功率计 CSV 先传到 131）
  B. 脚本跑在机房里采功率的那台 Windows PC 上
     - host 用 '10.151.83.131'，且需放通 131 的 MySQL 远程访问（3306 + root 远程授权）
     - CSV 用 Windows 路径，如 D:\master.csv
"""
...
# 三个 CSV 文件的完整路径
# 方案A(跑在131/Linux)示例: '/root/power/master.csv'
# 方案B(跑在Windows PC)示例: r'D:\master.csv'
CSV_FILES = {
...
# MySQL 连接信息（甲方 131 上的 bd_monitor）
# 方案A: host='localhost'  方案B: host='10.151.83.131'
DB_CONFIG = {
    'host':     'localhost',
    'port':     3306,
    'user':     'root',
    'password': 'Openthekey@2025',   # 甲方 131 MySQL root 密码
    'database': 'bd_monitor',
    'charset':  'utf8mb4',
}
