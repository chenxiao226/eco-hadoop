...
from power_logs_process import ActivePowerDataset
...
class Trainer:
    ...
    def build_data_loader(self):
        data_frame = pd.read_csv(args.CSV_PATH, encoding='utf-8')
    ...
    def test(
        ...
        for i, (batch_features, batch_targets) in enumerate(data_loader):
            ...
            print("✅ 推理完成：")
            print(f"mse_loss: {total_mse_loss.item() / total_num:.6f}, bce_loss: {total_bce_loss.item() / total_num:.6f}, relative_error: {total_relative_error.item() / total_num:.6f}")
            if (i + 1) % 5 == 0:
        ...
        # plt.close()
        # 画图
        out = all_inverse_outputs.flatten().cpu().numpy()
        tgt = all_inverse_targets.flatten().cpu().numpy()
        plt.rcParams['font.sans-serif'] = ['SimHei']  # 设置字体为黑体（SimHei 支持中文）
        plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示为方块的问题

        plt.figure(figsize=(15, 6))
        plt.plot(out, label="Predicted", c='red')
        plt.plot(tgt, label="True", c='green')
        plt.legend()
        plt.title("jenkins_web_预测值vs真实值")
        plt.xlabel("样本")
        plt.ylabel("功率")
        plt.savefig(self.output_dir + "/compare_prediction_truth.jpg")
        plt.close()

        mse_value = total_mse_loss.item() / total_num
        bce_value = total_bce_loss.item() / total_num
        rel_error = total_relative_error.item() / total_num
        accuracy = 1 - rel_error

        summary_text = (
            f"\n📊 模型评估结果：\n"
            f"  🔹 均方误差（MSE）       : {mse_value:.6f}\n"
            f"  🔹 交叉熵损失（BCE）     : {bce_value:.6f}\n"
            f"  🔹 平均相对误差（RE）    : {rel_error:.6f}\n"
            f"  ✅ 预测准确度估算（1-RE）: {accuracy * 100:.2f}%\n"
        )
...
if __name__ == '__main__':
    ...
    if args.eval_only:
        ...
        print(f"🚀 开始训练，总轮数: {args.NUM_EPOCHS_1}，每 {cfg.EVAL_PERIOD_1} 轮评估一次")
        trainer.train()
