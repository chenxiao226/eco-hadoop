...
def build_freq_timeline_from_master_log(task_name):
    ...
    格式示例（中文时间戳 + SET）：
        2024年11月16日 11时35分40秒
        SET 800MHz
    或:
        Linux 5.4.0 ... SET 800MHz ...（同行）
    返回 master_CpuFreq_Time: {ts_int: freq_int}
    """
...
def preprocess_log(task_name, file_name, TASK_END_TIME, Hadoop_df, master_CpuFreq_Time):
    """
    解析 system_log/slave1/log.log：
    - 频率：在 master_CpuFreq_Time 中按时间戳匹配（slave 与 master 同步）
    - 如果 master 时间线为空，则从 slave 本身的 SET 行读取
    """
    preline = ''
    cpu_freq = -1
    time = ''
...
if __name__ == "__main__":
    ...
    for file_name in FILE_NAME_LIST:
        ...
        row_sums = Init_hadoop_runtime_df[run_cols].sum(axis=1) if run_cols[0] in Init_hadoop_runtime_df.columns else None
